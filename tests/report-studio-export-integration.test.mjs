...
test('v1 scope keeps only stable cover choices and defers advanced controls', () => {
  ...
  assert.match(types, /rawCoverImageSource === 'first_evidence_image' \? 'first_evidence_image' : 'none'/)
  assert.match(actions, /coverImageSource:s\(formData,'cover_image_source'\)==='first_evidence_image'\?'first_evidence_image':'none'/)
