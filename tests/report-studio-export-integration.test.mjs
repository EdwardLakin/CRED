import assert from 'node:assert/strict'
import { readFileSync } from 'node:fs'
import test from 'node:test'

const route = readFileSync('app/api/dashboard/sessions/[id]/report-pdf/route.ts', 'utf8')
const studio = readFileSync('src/features/branding/components/BrandingStudio.tsx', 'utf8')
const types = readFileSync('src/features/branding/types.ts', 'utf8')
const actions = readFileSync('src/features/branding/actions.ts', 'utf8')
const templates = readFileSync('src/features/branding/templates.ts', 'utf8')
const dashboard = readFileSync('app/dashboard/page.tsx', 'utf8')
const settings = readFileSync('app/dashboard/settings/page.tsx', 'utf8')
const brandCss = readFileSync('src/features/report-studio/rendering/report-brand-css.ts', 'utf8')
const shell = readFileSync('src/features/report-studio/rendering/report-shell.ts', 'utf8')
const cover = readFileSync('src/features/report-studio/rendering/report-cover.ts', 'utf8')
const footer = readFileSync('src/features/report-studio/rendering/report-footer.ts', 'utf8')
const watermark = readFileSync('src/features/report-studio/rendering/report-watermark.ts', 'utf8')
const signatures = readFileSync('src/features/report-studio/rendering/report-signatures.ts', 'utf8')

test('selected, workspace default, system fallback, and old template rows normalize for export', () => {
  assert.match(route, /requestedTemplateId/)
  assert.match(route, /normalizeReportTemplate\(selectedTemplate\)/)
  assert.match(route, /normalizeReportTemplate\(defaultTemplate\)/)
  assert.match(route, /normalizeBrandProfile\(legacyBranding as any\)/)
  assert.match(route, /requestedTemplateId !== "system"/)
  assert.match(route, /normalizeBrandProfile\(\s*[\s\S]*exportBranding \?\? null,?\s*\)/)
  assert.match(types, /rawHeader === 'classic'/)
  assert.match(types, /row\?\.report_style \?\? \{\}/)
})

test('exported report receives classes, tokens, typography, header and evidence layout settings', () => {
  for (const token of ['theme-','typography-','header-','footer-','section-','evidence-','image-','signature-']) assert.match(shell, new RegExp(token))
  for (const token of ['--brand-primary','--brand-header-bg','--brand-footer-text','--brand-section-heading','--brand-evidence-accent']) assert.match(brandCss, new RegExp(token))
  assert.match(brandCss, /TYPOGRAPHY_OPTIONS\[brand\.typography\.preset\]/)
  assert.match(shell, /brand\.header_layout/)
  assert.match(shell, /brand\.report_style\.evidenceStyle/)
  assert.match(shell, /brand\.report_style\.evidenceImageSize/)
})

test('cover page, footer, watermark, signature, appendix, and metadata toggles affect report output', () => {
  assert.match(cover, /style\.coverPage === "none"\) return ""/)
  assert.match(cover, /style\.showCoverLogo/)
  assert.match(cover, /style\.showCoverTitle/)
  assert.match(route, /buildPrintFooterHtml/)
  assert.match(footer, /brand\?\.footer_text/)
  assert.match(shell, /buildWatermarkHtml/)
  assert.match(watermark, /watermark\.option === "none"/)
  assert.match(signatures, /show_signature_block === false/)
  assert.match(signatures, /signatureBlocks\?\.filter\(\(block\) => block\.enabled\)/)
  assert.match(route, /function shouldIncludeEvidenceAppendix/)
  assert.match(route, /if \(includeAppendix\)/)
  assert.match(route, /title: "Source Index"/)
  assert.match(route, /style\.timestamps \? \["Captured"/)
  assert.match(route, /style\.captureMetadata \? \["Type"/)
  assert.match(route, /style\.evidenceIds \? \["Item reference"/)
})

test('custom field definitions only render when report values exist', () => {
  assert.match(cover, /buildCustomFieldRows/)
  assert.match(cover, /draft\?\.report_structure\.custom_fields/)
  assert.match(cover, /typeof value === "string" && value\.trim\(\)/)
})

test('preview modes stay realistic, iPad friendly, and controls use readable labels', () => {
  for (const mode of ['cover page','header','report section','item layout','footer','signature','full sample report']) assert.match(studio, new RegExp(mode))
  assert.match(studio, /preview-toggle/)
  assert.match(studio, /brand-preview-panel/)
  assert.doesNotMatch(studio, />[a-z]+[A-Z][A-Za-z]*</)
})


test('stable cover choices remain available alongside advanced templates', () => {
  assert.match(types, /COVER_PAGE_LAYOUTS = \[[^\]]*'none'[^\]]*'simple_cover'[^\]]*'professional_cover'/)
  assert.match(types, /rawCover === 'letterhead_cover'[\s\S]*'professional_cover'/)
  assert.match(types, /rawCover === 'image_cover'[\s\S]*'simple_cover'/)
  assert.match(types, /export function normalizeReportStyle/)
  assert.match(types, /COVER_IMAGE_SOURCES = \['none','first_evidence_image'\]/)
  assert.match(types, /normalizeCoverImageSource\(value: unknown\)[\s\S]*value === 'first_evidence_image' \? 'first_evidence_image' : 'none'/)
  assert.match(actions, /coverImageSource:normalizeCoverImageSource\(s\(formData,'cover_image_source'\)\)/)
  assert.match(templates, /normalizeBrandProfile/)
  assert.doesNotMatch(`${studio}\n${types}\n${actions}`, /selected_report_image/)
  assert.doesNotMatch(studio, /show_page_number|Show page number/i)
  assert.doesNotMatch(studio, /id="custom-fields"|Add custom field definition|Template marketplace|Brand assets library/i)
})

test('export copy avoids generated diagnostic or AI recommendation language', () => {
  assert.doesNotMatch(route, /diagnostic procedure documentation|Diagnostic Procedure Workspace/)
  assert.doesNotMatch(route, /AI diagnosis|AI recommendation|AI-generated diagnosis|AI-generated recommendation/i)
  assert.doesNotMatch(studio, /AI diagnosis|AI recommendation|font upload|raw html|custom css/i)
})

test('guardrails: no prohibited feature families and existing navigation scope remains', () => {
  const combined = `${route}\n${studio}\n${types}`
  assert.doesNotMatch(combined, /social export|reel export|AI branding generation|AI diagnosis|AI recommendation|custom css|raw html|font upload/i)
  assert.match(dashboard, /sessions/i)
  assert.match(settings, /Offline Home Screen/)
})
