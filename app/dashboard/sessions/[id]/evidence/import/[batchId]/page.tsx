...
export default async function EvidenceImportBatchReviewPage({ params }: { params: Promise<{ id: string; batchId: string }> }) {
  ...
  return <main className="page-shell dashboard-shell">
    <div className="section-header page-header"><div><Link href={`/dashboard/sessions/${session.id}/evidence/import`} className="secondary-link touch-target">← Import evidence</Link><h1>Import batch review</h1><p className="muted">{session.title}</p><Link href={`/dashboard/sessions/${session.id}/evidence/review`} className="button button-secondary touch-target">Open Review Queue</Link></div></div>
