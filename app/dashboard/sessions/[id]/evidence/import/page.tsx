...
export default async function BulkEvidenceImportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="page-shell dashboard-shell">
    <div className="section-header page-header"><div><Link href={`/dashboard/sessions/${session.id}/evidence`} className="secondary-link touch-target">← Items</Link><h1>Import files</h1><p className="muted">{session.title}</p></div></div>
