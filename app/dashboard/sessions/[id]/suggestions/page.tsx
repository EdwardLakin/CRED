...
export default async function SuggestionsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  requireWorkspaceFeatureOrRedirect(workspace.profile, 'suggestions', id)
  const data = await getSuggestionsData(id, workspace)
  return <main className="page-shell dashboard-shell"><div className="section-header page-header"><div><Link href={`/dashboard/sessions/${data.session.id}`} className="secondary-link touch-target">← Session</Link><h1>Suggestions</h1><p className="muted">{data.session.title} · AI-generated suggestions require human review</p></div></div><EvidenceWorkspaceBacklinks accessSubject={workspace.profile} sessionId={data.session.id} current="suggestions" /><SuggestionsWorkspace data={data} /></main>
}
