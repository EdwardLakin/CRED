...
function buildFieldServiceReportHtml({
  ...
  <style>${REPORT_STYLES}</style></head><body><main class="report"><div class="toolbar"><button onclick="window.print()">Print / Save as PDF</button><p class="print-help">Use your browser’s Print or Share menu to save as PDF.</p></div><header class="header"><p class="eyebrow">Printable Report</p><h1>${escapeHtml(session.title)}</h1><p>${escapeHtml(organizationName)}</p><p class="meta">Documentation-only service report · ${escapeHtml(new Date().toLocaleDateString())}</p>${renderDefinitionRows(headerRows)}</header>${renderFieldServiceSection(details, 'equipment')}<section class="item service-section"><h2>Travel</h2>${renderDefinitionRows(travelRows)}</section><section class="item service-section"><h2>Work performed</h2>${renderDefinitionRows(workRows)}</section><section class="item service-section"><h2>Evidence</h2><p class="muted">Evidence items reference captured photos, videos, documents, and technician notes.</p></section>${evidenceHtml || '<section class="item"><h2>No report evidence selected.</h2></section>'}<section class="item service-section"><h2>Time card summary</h2>${renderDefinitionRows(timeRows)}</section><section class="item service-section"><h2>Charges / documentation only</h2>${renderDefinitionRows(chargeRows)}</section><section class="item service-section"><h2>Signature requirements</h2>${renderDefinitionRows(signatureRows)}</section>${signaturesHtml}</main></body></html>`
}
...
export async function GET(_request: Request, { params }: RouteContext) {
  ...
  <style>${REPORT_STYLES}</style></head><body><main class="report"><div class="toolbar"><button onclick="window.print()">Print / Save as PDF</button><p class="print-help">Use your browser’s Print or Share menu to save as PDF.</p></div><header class="header"><p class="eyebrow">Printable Report</p><h1>${escapeHtml(session.title)}</h1><p>${escapeHtml(organizationName)}</p><p class="meta">${escapeHtml(session.session_type)} · ${escapeHtml(assetDetails || 'No asset details')} · ${escapeHtml(new Date().toLocaleDateString())}</p></header>${itemsHtml || '<section class="item"><h2>No report evidence selected.</h2></section>'}${buildSignaturesHtml(reportSignatures, signatureUrls)}</main></body></html>`
