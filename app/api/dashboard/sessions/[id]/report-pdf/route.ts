...
type ExportBranding = { display_name?: string | null; tagline?: string | null; phone?: string | null; email?: string | null; website?: string | null; address?: string | null; prepared_by_name?: string | null; prepared_by_title?: string | null; logo_storage_path?: string | null; signature_storage_path?: string | null; colors?: Record<string,string>; typography?: { preset?: string }; header_layout?: string | null; footer_text?: string | null; show_report_id?: boolean; show_contact_info?: boolean; show_confidentiality_note?: boolean; show_signature_block?: boolean };
...
function buildReportCoverHtml(params: {
  ...
  const identityHtml = brand ? `<div class="brand-report-identity">${logoHtml}<div><strong>${escapeHtml(brandName || params.organizationName)}</strong>${brand.tagline ? `<p>${escapeHtml(brand.tagline)}</p>` : ""}<p>${escapeHtml([brand.phone, brand.email, brand.website].filter(Boolean).join(" · "))}</p>${brand.address ? `<p>${escapeHtml(brand.address)}</p>` : ""}</div></div>` : "";
  return `<section class="report-cover item branded-cover branded-cover-${escapeHtmlAttributeRaw(brand?.header_layout || "classic")}${coverImageHtml ? "" : " report-cover-no-image"}"><div class="cover-copy">${identityHtml}<div class="cover-kicker"><span>Documentation Report</span><span>${escapeHtml(params.reportType)}</span></div><h1>${escapeHtml(params.reportTitle)}</h1><p class="cover-trust">${escapeHtml(brand?.tagline || "Report identity and approved customer-facing documentation.")}</p>${renderDefinitionRows(rows)}</div>${coverImageHtml}</section>`;
}
