...
function shouldIncludeEvidenceAppendix(session: ReportSession, branding: ExportBranding | null | undefined) {
  ...
  return branding?.report_style.evidenceAppendix === true;
}
...
function buildExecutiveSnapshot(params: {
  ...
  const items: FinalReportItem[] = getDocumentedItemEntries(
    ...
    return {
      ...
      severity: getItemSeverityLabel(capture.severity),
      details,
...
function buildEvidenceAppendixHtml(
  ...
  return `<section class="item service-section evidence-appendix-section"><h2>Source Index</h2><p class="muted">Optional source references retained with the reviewed report.</p><div class="evidence-appendix-grid">${captures
    .map((capture) => {
      ...
      const title = getPrimaryEvidenceLabel(capture);
      const thumbnailHtml =
        options.renderImages !== false && mediaKind === "image"
          ? `<div class="appendix-thumb">${renderExportImage(imageAsset, title, "Image unavailable")}</div>`
          : options.renderImages === false
            ? ""
      ...
      return `<article class="appendix-card"${options.showDebugDetails ? ` data-capture-id="${escapeHtmlAttributeRaw(capture.id)}"` : ""}>${thumbnailHtml}<div class="appendix-copy"><h3>${escapeHtml(title)}</h3>${metadataRows.length ? `<dl>${metadataRows.map(([label, value]) => `<div><dt>${escapeHtml(label)}</dt><dd>${escapeHtml(value)}</dd></div>`).join("")}</dl>` : ""}</div></article>`;
    })
    .join("")}</div></section>`;
}
