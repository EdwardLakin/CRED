...
function customerText(value: unknown) {
  return stripConfidenceText(String(value ?? '')).replace(/Capture ID\s*:?\s*/gi, 'Evidence ').replace(UUID_PATTERN, 'evidence item')
}
...
function cleanReportTitle(preferred: string | null | undefined, session: ReportSession, draft: ReportDraft | null | undefined, genericFallback = false) {
  return getDisplayReportTitle(preferred ? { ...draft, title: preferred } : draft, session, { genericFallback })
}
...
function buildEvidenceAppendixHtml(captures: ReportCapture[], signedUrls: Record<string, string>, timeZone: string | null, options: { showDebugDetails?: boolean } = {}) {
  ...
  return `<section class="item service-section"><h2>Evidence Appendix</h2><p class="muted">Included captures are listed once from the reviewed report state. Each section traces back to capture IDs where available.</p><div class="evidence-grid">${captures.map((capture) => {
    ...
    const detailRows = [{ label: 'Evidence ID', value: evidenceMeta?.evidenceId ?? '' }, { label: 'Captured', value: evidenceMeta?.capturedAtLabel ?? formatDateTimeInTimeZone(new Date(capture.captured_at), timeZone) }, { label: 'Evidence type', value: evidenceMeta?.evidenceType ?? mediaKind }, { label: 'Source capture ID', value: options.showDebugDetails ? capture.id : '' }]
...
export async function GET(_request: Request, { params }: RouteContext) {
  ...
  const isGenericEvidenceReport = getFormStructureSummary(reportDraft?.report_structure ?? null, formSections).source === 'generic_fallback'
  ...
  const referenceHtml = reviewDocument.referenceDocuments.length ? buildReferenceDocumentsHtml(reviewDocument.referenceDocuments, signedUrls, { includeOriginal: false }) : ''
