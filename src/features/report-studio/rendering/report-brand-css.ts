...
export function buildBrandCss(branding?: ExportBranding | null) {
  ...
  const area = "areaStacks" in brand.typography && typeof brand.typography.areaStacks === "object" && brand.typography.areaStacks ? brand.typography.areaStacks as Record<string, string> : {};
  return `
    body{font-family:${area.body_text ?? type.bodyStack};color:${colors.accent}}
    ...
    .report h1,.report h2,.report h3{font-family:${area.section_headings ?? type.headingStack};font-weight:${type.headingWeight}}
    .cover-copy{font-family:${area.cover_page ?? type.headingStack}}.cover-copy h1{font-weight:${type.titleWeight};letter-spacing:${type.titleSpacing}}
