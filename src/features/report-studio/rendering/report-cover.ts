...
export function buildReportCoverHtml(params: {
  ...
  const allRows = [
    ...
    { key: "asset", label: "Asset / Equipment", value: getReportInfoValue(params.draft, params.session, "asset_equipment") || params.session.asset_label || params.session.unit_number || "" },
    { key: "location", label: "Location", value: getReportInfoValue(params.draft, params.session, "location") },
  ...
  return `<section style="${coverVars}" class="report-cover item branded-cover branded-cover-${escapeHtmlAttributeRaw(brand?.header_layout || "classic")} cover-${escapeHtmlAttributeRaw(style.coverPage)} logo-${escapeHtmlAttributeRaw(style.coverLogoSize)}${coverImageHtml ? "" : " report-cover-no-image"}"><div class="cover-copy">${identityHtml}<div class="cover-kicker"><span>Executive Report</span><span>${escapeHtml(params.reportType)}</span></div>${style.showCoverTitle ? `<h1>${escapeHtml(params.reportTitle)}</h1>` : ""}${params.helpers.renderDefinitionRows(definitionRows)}</div>${coverImageHtml}</section>`;
}
