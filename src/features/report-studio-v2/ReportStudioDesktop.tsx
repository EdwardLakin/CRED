...
export function ReportStudioDesktop(props: ReportStudioProps & { state: any; handlers: any }) {
  ...
  return <main className="rsv2-shell" data-report-studio-v2="desktop"><ReportStudioToolbar props={props} state={props.state} handlers={props.handlers} onTemplates={openTemplates} onPalettes={openPalettes} onTemplateSaved={upsertSavedTemplate}/><div className="rsv2-body"><ReportPreviewCanvas brand={props.state.draftBrandProfile} session={props.state.selectedSession} activeSection={props.state.activeSection} setActiveSection={props.handlers.setActiveSection} assets={props}/><ReportControlPanel brand={props.state.draftBrandProfile} patchBrand={props.handlers.patchBrand} activeSection={props.state.activeSection} setActiveSection={props.handlers.setActiveSection}/></div>{drawerMode&&<ReportTemplateDrawer mode={drawerMode} templates={savedTemplates} onClose={()=>setDrawerMode(null)} onApply={(t:any,id?:string)=>{props.handlers.applyTemplate(t,id??t.id);setDrawerMode(null)}} onApplyDefaultPalette={(t:any,id?:string)=>{props.handlers.applyTemplateDefaultPalette(t,id??t.id);setDrawerMode(null)}} onApplyPalette={(name:string,colors:any)=>props.handlers.applyPalette(name,colors)} defaultTemplateId={savedTemplates.find((t:any)=>t.is_default)?.id ?? null}/>}</main>;
}
