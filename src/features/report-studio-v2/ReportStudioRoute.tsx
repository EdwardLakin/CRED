...
export function ReportStudioRoute(props: ReportStudioProps) {
  ...
  const applyTemplate = (template: WorkspaceBrandProfile, id: string) => { if (!id) return; const currentColors = draftBrandProfile.colors; const next = normalizeBrandProfile(template); const preservesColors = id.startsWith("system:") || id.startsWith("preset:"); patchBrand(preservesColors ? { ...next, colors: currentColors } : next); setSelectedTemplateId(id); if (!preservesColors) setSelectedPaletteName(null); };
  const applyTemplateDefaultPalette = (template: WorkspaceBrandProfile, id: string) => { if (!id) return; patchBrand(normalizeBrandProfile(template)); setSelectedTemplateId(id); setSelectedPaletteName(null); };
