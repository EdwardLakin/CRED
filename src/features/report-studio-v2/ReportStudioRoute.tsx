...
export function ReportStudioRoute(props: ReportStudioProps) {
  ...
  const selectedSession = useMemo(() => { const session = props.sessions.find((s) => s.id === selectedSessionId) ?? props.sessions[0] ?? null; return session ? { ...session, report_summary: sessionSummaries[session.id] ?? session.report_summary ?? null } : null; }, [props.sessions, selectedSessionId, sessionSummaries]);
