...
export function sanitizeEvidenceFilename(name: string) { const fallback = 'evidence-file'; const cleaned = name.replace(/[/\\]/g, '-').replace(/[^\w. -]+/g, '').replace(/\s+/g, ' ').trim(); return (cleaned || fallback).slice(0, 160) }
