...
export function EvidenceImportBatchDetail({ sessionId, batch, captureItems, timeZone }: { sessionId: string; batch: EvidenceImportBatch; captureItems: EvidenceImportCaptureItem[]; timeZone: string | null }) {
  return <section className="card detail-card form-stack">
    <div className="section-header"><div><p className="eyebrow">Import batch</p><h2>{batch.status}</h2></div><Link className="button button-secondary" href={`/dashboard/sessions/${sessionId}/evidence`}>Items</Link></div>
    ...
    {captureItems.length === 0 ? <p className="muted">No items were created for this batch.</p> : <ul className="metadata-list">{captureItems.map((item) => <li key={item.id}><Link href={`/dashboard/sessions/${sessionId}/evidence/${item.id}`}>{item.original_filename ?? 'Imported item'}</Link> · {item.source_kind} · {item.processing_status} · {item.evidence_review_status}</li>)}</ul>}
  </section>
}
