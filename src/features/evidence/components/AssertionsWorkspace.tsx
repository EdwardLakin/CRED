...
function evidenceLabel(item?: AssertionEvidenceItem) { return item ? item.original_filename || item.technician_note || `${item.media_kind} evidence` : 'Unknown evidence' }
...
function RelationshipList({ sessionId, relationships, evidenceItems, entities, timelineEvents }: { sessionId: string; relationships: AssertionRelationship[]; evidenceItems: AssertionEvidenceItem[]; entities: AssertionEntity[]; timelineEvents: AssertionTimelineEvent[] }) {
  if (relationships.length === 0) return <p className="muted">No linked evidence, entities, or timeline events.</p>
