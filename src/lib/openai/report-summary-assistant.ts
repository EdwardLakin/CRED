...
export async function regenerateReportSummaryFromEvidence(input: { sessionTitle: string | null; evidence: SummaryAssistantEvidence[] }) {
  ...
  if (!evidence.length) throw new Error('No documented observations are available for summary regeneration.')

  return requestSummaryAssistant(
    `You write a customer-facing CRED executive summary from already documented report observations/evidence. Return JSON only. Do not invent unsupported facts. Do not invent severity, causes, liability language, recommendations, dates, names, numbers, measurements, IDs, VINs, codes, or technical values. Only mention recommendations or severity when explicitly present in the provided documented text. Use neutral wording and preserve source limitations.`,
    `Report title: ${input.sessionTitle ?? 'Untitled report'}\nDocumented observations/evidence JSON:\n${JSON.stringify(evidence).slice(0, 30000)}`,
  )
}
