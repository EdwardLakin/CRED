...
const getDisplaySessionType = (value) => value === 'General Evidence Report' ? 'General Documentation Report' : value;
...
function recordController(label) { const checkpoint = { label, at: new Date().toISOString(), controlled: Boolean(navigator.serviceWorker?.controller), scriptURL: navigator.serviceWorker?.controller?.scriptURL ?? null }; logReadiness(`controller ${label}`, checkpoint); updateSw({ controlled: checkpoint.controlled, activeScriptURL: checkpoint.scriptURL || state.serviceWorker.activeScriptURL, controllerCheckpoints: [...state.serviceWorker.controllerCheckpoints, checkpoint] }); }
async function snapshotRegistrations(primary) { const registrations = await navigator.serviceWorker.getRegistrations().catch(() => primary ? [primary] : []); const snapshots = registrations.map(registrationSnapshot); const scoped = primary && !snapshots.some((item) => item.scope === primary.scope) ? [registrationSnapshot(primary), ...snapshots] : snapshots; const scopes = scoped.map((item) => item.scope); updateSw({ registrations: scoped, registrationCount: scoped.length, duplicateRegistrations: new Set(scopes).size !== scopes.length, activeScriptURL: primary?.active?.scriptURL || scoped.find((item) => item.active.scriptURL)?.active.scriptURL || null }); logReadiness('registrations', scoped); return scoped; }
function syncWorkerState(worker) { if (!worker)
    return; const isInstalling = worker.state === 'installing'; updateSw({ installing: isInstalling, installed: worker.state === 'installed' || worker.state === 'activated', activating: worker.state === 'activating', activated: worker.state === 'activated', installingState: isInstalling ? worker.state : state.serviceWorker.installingState, installingScriptURL: isInstalling ? worker.scriptURL : state.serviceWorker.installingScriptURL }); }
...
async function waitForOfflineAssets(requiredAssets = REQUIRED_OFFLINE_ASSETS, timeoutMs = TIMEOUTS.cache, intervalMs = TIMEOUTS.cachePoll) { const expires = Date.now() + timeoutMs; let last = { cached: false, missing: requiredAssets, cacheNames: [] }; updateSw({ finalStatus: 'Installing offline assets…', error: null }); while (Date.now() < expires) {
    ...
    updateSw({ cacheNames: last.cacheNames || [], cached: last.cached, cacheMissing: last.missing || requiredAssets, finalStatus: last.cached ? 'Offline assets installed.' : 'Installing offline assets…', error: last.error || null });
    if (last.cached)
        return last;
    await delay(intervalMs);
} last = await requiredAssetsCached(requiredAssets).catch((error) => ({ cached: false, missing: requiredAssets, cacheNames: [], error: error instanceof Error ? error.message : String(error) })); return { ...last, timedOut: !last.cached }; }
function detectStuckInstalling(registration, timeoutMs = TIMEOUTS.workerState) { const worker = registration.installing; if (!worker)
    return; const startedAt = new Date().toISOString(); updateSw({ installingState: worker.state, installingScriptURL: worker.scriptURL, installingLastStateChangeAt: startedAt, installStuck: false }); window.setTimeout(() => { if (registration.installing === worker && worker.state === 'installing' && !registration.waiting && !registration.active)
    updateSw({ installStuck: true, installing: true, installingState: worker.state, installingScriptURL: worker.scriptURL, installingLastStateChangeAt: state.serviceWorker.installingLastStateChangeAt || startedAt, finalStatus: 'Service worker install is stuck.', error: 'Service worker install is stuck.' }); }, timeoutMs); }
function waitForWorkerState(registration, targetStates, timeoutMs = TIMEOUTS.workerState) { const workers = () => [registration.installing, registration.waiting, registration.active].filter(Boolean); if (workers().some((worker) => targetStates.includes(worker.state)))
    return Promise.resolve(true); logReadiness(`waiting for worker states ${targetStates.join(', ')}`); return new Promise((resolve) => { const workerListeners = []; let done = false; const cleanup = () => { window.clearTimeout(timeout); registration.removeEventListener('updatefound', onUpdateFound); for (const [worker, listener] of workerListeners)
    worker.removeEventListener('statechange', listener); }; const finish = (value) => { if (done)
    return; done = true; cleanup(); resolve(value); }; const watch = (worker) => { if (!worker)
    return; syncWorkerState(worker); if (targetStates.includes(worker.state))
    finish(true); const listener = () => { syncWorkerState(worker); if (targetStates.includes(worker.state))
...
async function ensureServiceWorkerControl() { if (!('serviceWorker' in navigator)) {
    ...
    detectStuckInstalling(registration, TIMEOUTS.workerState);
    await snapshotRegistrations(registration);
    ...
    try {
        ...
        updateSw({ readyResolved: true, readyTimedOut: false, readyRejected: null, activated: Boolean(readyRegistration.active || registration.active) });
        logReadiness('navigator.serviceWorker.ready resolved', registrationSnapshot(readyRegistration));
        recordController('after ready');
        await snapshotRegistrations(readyRegistration);
    }
    catch (error) {
        updateSw({ readyResolved: false, readyTimedOut: true, readyRejected: error instanceof Error ? error.message : String(error) });
        logReadiness('navigator.serviceWorker.ready timed out', error);
    }
    const cacheState = await waitForOfflineAssets(REQUIRED_OFFLINE_ASSETS, TIMEOUTS.cache, TIMEOUTS.cachePoll);
    await workerProgress;
    let controlled = Boolean(navigator.serviceWorker.controller);
    if (!controlled)
        controlled = await waitForControllerChange();
    recordController('after controller wait');
    ...
    if (!cacheState.cached) {
        updateSw({ ...base, finalStatus: 'Offline asset installation timed out.', error: cacheState.error || 'Offline asset installation timed out.' });
        return state.serviceWorker;
    }
    if (!controlled) {
        const reloaded = sessionStorage.getItem(CONTROL_RELOAD_KEY) === 'true';
        const reason = state.serviceWorker.scopeMatchesPage === false ? 'Incorrect registration scope for this page.' : state.serviceWorker.waiting ? 'Service worker is waiting despite skipWaiting.' : state.serviceWorker.installing ? 'Service worker is still installing.' : state.serviceWorker.activated ? (reloaded ? 'Worker active but Safari has not attached control after reload.' : 'Worker active; Safari may require one navigation/reload before control attaches.') : 'Service worker installed but this browser has not attached control yet.';
        updateSw({ ...base, finalStatus: reason, error: null });
        return state.serviceWorker;
    }
    sessionStorage.removeItem(CONTROL_RELOAD_KEY);
    updateSw({ ...base, finalStatus: 'Offline assets installed.', error: null });
    return state.serviceWorker;
}
catch (error) {
...
async function testOfflineReload() { updateSw({ offlineReloadTest: 'Testing /offline.html…' }); try {
    const cached = await caches?.match('/offline.html').catch(() => undefined);
    if (cached?.ok) {
        updateSw({ offlineReloadTest: 'PASS: /offline.html is already available from Cache Storage for offline reload.' });
        return;
    }
    const response = await fetch('/offline.html', { cache: 'reload', headers: { Accept: 'text/html' } });
    const contentType = response.headers.get('content-type') || '';
    if (response.ok && contentType.includes('text/html'))
        updateSw({ offlineReloadTest: 'PASS: /offline.html returns 200 text/html and can be cached by the service worker.' });
    else
        updateSw({ offlineReloadTest: `FAIL: /offline.html returned ${response.status} ${contentType || 'without content-type'}.` });
}
catch (error) {
    updateSw({ offlineReloadTest: `FAIL: /offline.html is not reachable: ${error instanceof Error ? error.message : String(error)}` });
} }
function offlineDebugEnabled() { return new URLSearchParams(window.location.search).get('debugOffline') === '1' || localStorage.getItem('cred-offline-debug') === '1'; }
...
async function moveCapture(captures, index, direction) {
    ...
    await Promise.all(captures.map((capture, index) => updateCapture(capture, { metadata: { ...capture.metadata, reportOrder: index + 1 } })));
    await renderWorkspace();
}
